...
class Encoder(nn.Module):
    def __init__(self, size, dim=512, dim_motion=20):
        ...
        # motion network
        fc = [EqualLinear(dim, dim, activation='fused_lrelu')]
        for i in range(3):
            fc.append(EqualLinear(dim, dim, activation='fused_lrelu'))

        fc.append(EqualLinear(dim, dim_motion, activation='fused_lrelu'))
        self.fc = nn.Sequential(*fc)
    ...
    def forward(self, input_source, input_target, h_start=None):
        ...
        if input_target is not None:
            ...
            # h_target, target_feats = self.net_app(input_target)
            ...
            # print('h_motion_target', h_motion_target.shape)
            ...
            # return h_source, h_motion, feats, target_feats
        else:
            # print('2')
            h_source, feats = self.net_app(input_source)

            return h_source, None, feats
            # return h_source, None, feats, target_feats